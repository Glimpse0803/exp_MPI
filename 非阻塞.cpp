...
void parallelSolution_3(int N) {
    ...
    for(int k = 0; k < N; k++ ) {
        ...
        if(myid == source) {
            ...
            A[k][k] = 1;
            MPI_Isend(&A[p][0], N, MPI_FLOAT, down, k, MPI_COMM_WORLD, &request);
            p++;
        } else {
			MPI_Irecv(&A[p][0], N, MPI_FLOAT, up, MPI_ANY_TAG, MPI_COMM_WORLD, &request);
            MPI_Wait(&request, &status);
            int picked = status.MPI_TAG;
			if (down != source) {
                MPI_Isend(&A[p][0], N, MPI_FLOAT, down, picked, MPI_COMM_WORLD, &request);
            }
		}
        for (int i = p; i < num; ++i) {
            for(int j = k+1; j < N; j++) {
...
int main(int argc, char *argv[]) {
    ...
    for (int i = step; i <= N; i += step) {
		...
		if(myid == 0) {
        	...
        	cout << setw(13) << "数组规模" <<  i << ": " << "普通MPI平均用时：" << setw(20) << time1 << endl;
        	cout << setw(13) << " " << "流水线优化MPI平均用时：" << setw(20) << time2 << endl;
        	cout << endl;
		}
	}
	MPI_Finalize();
